// Package probe is cmd/probe's logic made IMPORTABLE (#77 proof): the same route-
// verification, but as func Run([]string) int so cmd/eight can call it IN-PROCESS
// (no exec) — the mechanism the one-binary merge needs. cmd/probe/main.go is now a
// thin wrapper over this. Behaviour is identical to the old cmd/probe.
package probe
...
func probeRoute(client *http.Client, base, method, path string) (string, int, string) {
	var body io.Reader
...
// Run is the importable entrypoint (was func main). Returns a process exit code.
func Run(args []string) int {
	if len(args) != 2 {
		fmt.Fprintln(os.Stderr, "usage: probe <base_url> <label>")
		return 1
	}
	base, label := strings.TrimRight(args[0], "/"), args[1]
	wd, _ := os.Getwd()
	...
	for _, k := range keys {
		verdict, code, errStr := probeRoute(client, base, k.Method, k.Path)
		tally[verdict]++
		claimed := routes[k]
